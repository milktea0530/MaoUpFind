...
{
    public class Adoption01Controller : Controller
    {
        ...
        public ActionResult Index()
        {
            ...
            if (Session[CDictionary.SK_LOGINED_ID] != null)
            {
                ...
                (new F_Adoption01()).GetMemberInfo(ref model, s_id, s_type);
            }
            return View(model);
        }

        [HttpPost]
        public ActionResult Index(VM_Adoption01_Form data)
        {
            ...
            // 交給工廠寫入資料庫
            var rInfo = (new F_Adoption01()).Create(ref data);
            // 失敗 -> 回到Index
            if (!rInfo.isSuccess)
            {
            ...
            // 回傳單號+編號            
            Session["adoption"] = data.adoption;
            return RedirectToAction("addSuccess");
        }
        public ActionResult addSuccess()
        {
            var adoption = (刊登認養)Session["adoption"];
