...
namespace MaoUpFind.Factories
{
    public class F_Bulletin01
    {
        ...
        /// <returns></returns>
        public void GetMemberInfo(ref VM_Bulletin01_Form data, string s_id, string s_type)
        {
        ...
        /// <returns></returns>
        public VM_Info Create(ref 通報資料 data)
        {
            VM_Info rInfo = new VM_Info();
            // 建立單號, 通報時間, 是否結案, 建立時間, 更新時間
            // 單號規則 B + 年月日 + 亂數
            ...
            data.通報單號 = "B" + s_date + num;

            try
            {
                MaoUpFindEntities db = new MaoUpFindEntities();
                // 處理地址
                //var tempCity = data.通報地點_市;
                //var tempArea = data.通報地點_區;
                //var city = db.城市.Where(m => m.城市編號 == tempCity).FirstOrDefault();
                //var area = db.地區.Where(m => m.地區編號 == tempArea).FirstOrDefault();
                //data.發生地點_全 = city.城市名稱 + area.地區名稱 + data.發生地點_全;
                // 寫入資料庫
                db.通報資料.Add(data);
                db.SaveChanges();
...
";
            ...
            flex = flex.Replace("--------取代標題--------", data.通報標題);
            flex = flex.Replace("--------取代地點--------", data.發生地點_全);
            flex = flex.Replace("--------取代時間--------", data.通報時間.ToString());
            flex = flex.Replace("--------詳細資料--------", "https://localhost:44364/Bulletin03/detailView?number=" + data.通報單號);
        ...
        #region 寄送通報成功通知(寄給刊登人)
        /// <summary>
        /// 寄送通報成功通知(寄給刊登人)
        /// </summary>
        /// <param name="data"></param>
        public void 寄送通報成功通知(通報資料 data)
        {
            MaoUpFindEntities db = new MaoUpFindEntities();
            VM_Mail m_Mail = new VM_Mail();
            // 取得會員的mail(找不到mail就不寄信)
            var user = db.會員檢視表.Where(m => m.會員編號 == data.會員編號).FirstOrDefault();
            if (user.會員信箱 == null) return;
            m_Mail.M_Address = user.會員信箱;
            string url = "https://localhost:44364/Bulletin03/detailView?number=" + data.通報單號;
            // 信件標題
            m_Mail.M_Subject = "建立通報成功通知";
            // 信件內容
            m_Mail.M_Body += "<html><body>";
            m_Mail.M_Body += $"<p>您好 {user.會員帳號},您已成功建立通報</p>";
            m_Mail.M_Body += $"<p>通報單號: <span style='color:blue'>{data.通報單號}</span></p>";
            m_Mail.M_Body += $"<p>通報時間: {data.通報時間}</p>";
        ...
        #region 寄送通報成功通知(寄給符合地區的會員)
        /// <summary>
        /// 寄送通報成功通知(寄給符合地區的會員)
        /// </summary>
        /// <param name="data"></param>
        public void 寄送最新通報資訊(通報資料 data)
        {
            MaoUpFindEntities db = new MaoUpFindEntities();
            VM_Mail m_Mail = new VM_Mail();
            // 取得所有符合地區的會員資料,設定可收到通知,排除刊登人
            var users = db.會員檢視表.Where(m => m.區 == data.通報地點_區
                                                && m.市 == data.通報地點_市
                                                && m.收到通知 == "1"
                                                && m.會員帳號 != data.會員編號.ToString()
                ).ToList();
            if (users.Count <= 0) return;
            string url = "https://localhost:44364/Bulletin03/detailView?number=" + data.通報單號;
            // 信件標題
            m_Mail.M_Subject = "最新通報資訊";
            // 信件內容
            m_Mail.M_Body += "<html><body>";
            m_Mail.M_Body += $"<p>您好 --取代會員帳號--</p>";
            m_Mail.M_Body += $"<p>您所設定的聯絡地區有最新動物通報</p>";
